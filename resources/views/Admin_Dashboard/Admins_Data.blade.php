                ...
                <table class="table table-hover">
                    ...
                    <tbody>
                        @foreach ($admins as $admin)
                            <tr>
                                ...
                                <td>
                                    <div>
                                        @if ($admin->image)
                                            <img src="{{ asset('images/users/' . $admin->image) }}"
                                                alt="{{ $admin->name }}" width="80" height="80">
                                        @endif
                                    </div>
                                </td>
                                <td>
                        ...
                        <button class="btn btn-warning" type="submit" value="Update" style="width:70px">View</button>
                                    ...
                                    <form style="display: inline-block;" action="admindelete/{{ $admin['id'] }}"
                                        method="POST" style="margin-bottom: 2px">
                                        @csrf
                                        @method('DELETE')
                                        <button style="height:40px" class="btn btn-danger" type="submit" value="DELETE"><i
                                                class="fa fa-trash" aria-hidden="true"></i></button>
                                    </form>
                                    <form style="display: inline-block" action="adminsedit/{{ $admin['id'] }}"
                                        method="get" style="margin-bottom: 2px">
